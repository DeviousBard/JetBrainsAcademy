...
class BufferedImageEnergyGrid(buffer: BufferedImage): Cloneable {
    ...
    private var grid = Array(cols) { Array(rows) { 0.0 } }
    ...
    private fun transpose(): BufferedImageEnergyGrid {
        val newGrid = this.clone() as BufferedImageEnergyGrid
...
class Seam(private val energyGrid: BufferedImageEnergyGrid) {
    private val cols = energyGrid.cols
    private val rows = energyGrid.rows
    private var grid = Array(cols) { Array(rows) { Double.POSITIVE_INFINITY }}
